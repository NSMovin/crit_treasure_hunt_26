...
export async function renderVotingManager(container, _unsubs) {
  ...
  if (sessionId) {
    const unsub = onPhotoVotesChange(sessionId, async () => {
      ...
      renderTable(container.querySelector('#vote-table-body'), fresh, gs?.voting_open ?? false);
    });
    _unsubs.push(unsub);
  }
}
...
function renderPanel(container, votingOpen, submissions, sessionId) {
  ...
  if (submissions.length > 0 && sessionId) {
    renderTable(container.querySelector('#vote-table-body'), submissions, votingOpen);
  }
...
function renderTable(tbody, submissions, votingOpen) {
  if (!tbody) return;
...
async function openVoting(container, sessionId) {
  ...
  try {
    ...
    const [gs, submissions] = await Promise.all([getGameState(), getPhotoSubmissions(sessionId)]);
